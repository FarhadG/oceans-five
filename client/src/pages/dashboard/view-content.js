...
const SalesDataChart = () => (
  <ResponsiveContainer>
    <AreaChart data={salesData} margin={{ top: 10, right: 10, left: -15, bottom: 0 }}>
      <XAxis dataKey="name" axisLine={false} fontSize={10} tickLine={false}
             padding={{ left: 0, right: 5 }} />
      <YAxis fontSize={10} axisLine={false} tickLine={false} />
      <CartesianGrid stroke="#eee" vertical={false} />
      <Tooltip wrapperStyle={{ borderColor: '#eee' }} />
      <Legend />
      <Area type='monotone' dataKey='iphone' stackId="1" strokeWidth={2} stroke="#448AFF"
            fill='#448AFF' fillOpacity=".8" />
      <Area type='monotone' dataKey='imac' stackId="1" strokeWidth={2} stroke="#69F0AE"
            fill='#69F0AE' fillOpacity=".8" />
    </AreaChart>
  </ResponsiveContainer>
);
...
class TrafficSourceChart extends React.Component {
  ...
  render() {
    return (
      <ResponsiveContainer>
        <PieChart>
          <Pie onMouseEnter={this.onPieEnter}
               data={trafficSourceData}
               activeIndex={this.state.activeIndex} activeShape={renderActiveShape}
               outerRadius={90}
               dataKey="value"
               innerRadius={70} paddingAngle={4}>
            {trafficSourceData.map((entry, index) => <Cell fill={trafficSourceColors[index]}
                                                           key={index} />)}
          </Pie>
          <Legend />
        </PieChart>
  ...
  <table className="table">
    ...
    {data.map((item, i) => <tr key={i}>
      ...
      <td className="align-middle text-nowrap">
        <IconDot size="12"
                 color={item.status === 'done' ? '#4CAF50' : '#F44336'} />&nbsp;{item.status}
      </td>
    </tr>)}
    </tbody>
  </table>
  ...
  <CardGroup className="sales-card mb-4">
    ...
    <Card>
      ...
      <CardBlock>
        ...
        <h4 className="font-weight-normal mb-0"><IconTrendUp size="16" color="#00c853" />21,32,520
        </h4>
      </CardBlock>
    </Card>
  </CardGroup>
        ...
        <Card>
          <CardBlock>
            ...
            <div className="d-flex mb-4 align-items-center">
              <IconAndroid size="52" style={{ 'border': '1px solid #eee' }}
            ...
            <div className="d-flex mb-4 align-items-center">
              <IconCardTravel size="52" style={{ 'border': '1px solid #eee' }}
            ...
            <div className="d-flex mb-4 align-items-center">
              <IconDvr size="52" style={{ 'border': '1px solid #eee' }}
            ...
            <div className="d-flex mb-4 align-items-center">
              <IconBalance size="52" style={{ 'border': '1px solid #eee' }}
      ...
      <div className="col-md-7 mb-4">
        <Card>
          <CardBlock className="table-responsive">
            ...
            <div className="small mb-4 card-subtitle"><Button color="primary"
                                                              size="sm">View all</Button></div>
